...
// TestMigrateAddsProjectTagsDefault 验证旧库项目迁移后补齐空标签数组。
func TestMigrateAddsProjectTagsDefault(t *testing.T) {
	dataDir := t.TempDir()
	dbPath := filepath.Join(dataDir, "webshare.db")
	db, err := sql.Open("sqlite", "file:"+filepath.ToSlash(dbPath))
	if err != nil {
	...
	_, err = db.Exec(`INSERT INTO projects(owner_id, name, slug, share_state, created_at, updated_at) VALUES(1, '旧项目', 'old-tags', 'public', ?, ?)`, now, now)
	if err != nil {
		t.Fatal(err)
	}
	if err := db.Close(); err != nil {
